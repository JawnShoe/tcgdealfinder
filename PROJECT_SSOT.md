...
- Security Patch Pack: Next 14.2.35 + React2Shell (2025-12-24): Pinned Next.js to exact version 14.2.35 (from ^14.2.33) to address 3 RSC/Server Action CVEs affecting Next 14.x stable: CVE-2025-55184 (high DoS), CVE-2025-55183 (medium source exposure), CVE-2025-67779 (high complete fix). CVE-2025-66478 (critical RCE) affects Next 15/16 + 14 canary only; Next 14 stable is not vulnerable. Ran npx fix-react2shell-next (reported no vulnerable packages). No runtime code changes. Build/lint verified passing (39 routes). Commits: 17ccf32, 41fed68. PR: [pending].
...
- Repo Hardening Pack D — Dependency Hygiene Automation (2025-12-24): Added GitHub Action to auto-merge safe Dependabot PRs (patch/minor updates only; major updates require manual review). Workflow enables auto-merge only when CI passes ("Lint & Build" required check) and semver indicates patch/minor. Major updates receive comment explaining manual review requirement. Monthly dependency sweep checklist added to SSOT. Workflow hardening commit added explicit guardrail documentation. Classification: CI automation (dependency safety gate). Commits: 67e853c, 5842a75. PR: [pending].

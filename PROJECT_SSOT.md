...
**ACTIVE WORK**: Track B — UI/SSR Stability Pass: Historic USD display preserved; made deterministic to avoid hydration mismatch; added USD baseline toggle (SSR-safe cookie) + Playwright guard (PR #109)
...
- Track B — UI/SSR stability pass (2025-12-29): Historic USD display preserved; made deterministic to avoid hydration mismatch on `/`, `/top-deals`, and `/watchlist` (no client-side currency conversion/repair for Historic). Added USD baseline toggle (SSR-safe cookie) + Playwright guard for converted USD line stability. PR: #109 (open).

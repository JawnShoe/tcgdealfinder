...
| Discovery preset support (`sort=endingSoon`)   | Canonical discovery supported `sort=newest` and `sort=biggest-discount` only                      | Canonical discovery accepts `sort=endingSoon` and the Sort control reflects the value                                                  | Stage 1 preset parity for `/ending-soon` decommission                       | PR #TBD; `lib/rebuild/prefs/rebuildPrefs.ts`; all checks green                                                                       |

...
| `/top-deals`                                   | Rendered legacy top-deals listing                                                                 | Server-side redirect to `/discovery?sort=biggest-discount` (top deals preset)                                                          | Canonical route cutover completed (post PR #314)                            | PR #315; `tests/e2e/rebuild.synthetics.spec.ts` (Stage 1 decommission test); all checks green                                        |
| `/newest`                                      | Rendered legacy newest listing                                                                    | Server-side redirect to `/discovery?sort=newest` (newest preset)                                                                       | Stage 1 visitor surface consolidation onto canonical discovery              | PR #316; `tests/e2e/rebuild.synthetics.spec.ts` (Stage 1 newest decommission test); all checks green                                 |
| `/ending-soon`                                 | Rendered legacy ending-soon listing                                                               | Server-side redirect to `/discovery?sort=endingSoon` (endingSoon preset)                                                               | Stage 1 visitor surface consolidation onto canonical discovery              | PR #318; `tests/e2e/rebuild.synthetics.spec.ts` (Stage 1 ending-soon decommission test); all checks green                            |
| Discovery preset support (`sort=endingSoon`)   | Canonical discovery supported `sort=newest` and `sort=biggest-discount` only                      | Canonical discovery accepts `sort=endingSoon` and the Sort control reflects the value                                                  | Stage 1 preset parity for `/ending-soon` decommission                       | PR #317; `lib/rebuild/prefs/rebuildPrefs.ts`; all checks green                                                                       |
| Canonical Discovery Routes (`/`, `/discovery`) | Discovery lived under `/rebuild/discovery` (and `/` redirected to `/rebuild`)                     | Canonical `/discovery` and `/` render discovery; `/rebuild/discovery` is a code-alias (no redirect)                                    | Option A cutover: remove `rebuild` namespace from public discovery surface  | PR #314; `tests/e2e/rebuild.synthetics.spec.ts` (rebuild synthetics: trust surfaces visible across rebuild funnel); all checks green |

...
async function checkStoreNames() {
  ...
  console.log(`  With store name: ${stat.with_store_name} (${((stat.with_store_name / stat.total) * 100).toFixed(1)}%)`);
  ...
  for (const s of samples.rows) {
    console.log(`  ${s.seller_username} → ${s.seller_store_name} (${s.source})`);
  }

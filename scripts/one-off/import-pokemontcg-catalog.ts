...
async function fetchWithAuth<T>(
  ...
  try {
    ...
    if (response.status === 429 || response.status >= 500) {
      if (retryCount < MAX_RETRIES) {
        const delay =
          ...
          Math.random() * 1000;
        console.log(
          `  ⚠️  ${response.status} error, retrying in ${Math.round(delay)}ms (attempt ${retryCount + 1}/${MAX_RETRIES})`
        );
      ...
      throw new Error(
        `API request failed after ${MAX_RETRIES} retries: ${response.status}`
      );
    }
    ...
    if (!response.ok) {
      const text = await response.text();
      throw new Error(
        `API request failed: ${response.status} ${text.substring(0, 200)}`
      );
    }
    ...
    if (
      ...
      const delay =
        INITIAL_RETRY_DELAY_MS * Math.pow(2, retryCount) + Math.random() * 1000;
      console.log(
        `  ⚠️  ${error.name}, retrying in ${Math.round(delay)}ms (attempt ${retryCount + 1}/${MAX_RETRIES})`
      );
      await new Promise((resolve) => setTimeout(resolve, delay));
      return fetchWithAuth<T>(url, retryCount + 1);
    }
...
async function fetchCardsForSet(
  ...
  while (hasMore) {
    ...
    console.log(
      `    Got ${response.data.length} cards (total: ${allCards.length}/${response.totalCount})`
    );
...
async function getImportProgress(): Promise<{
  lastSetIndex: number;
  totalSets: number;
}> {
...
async function importCatalog(): Promise<void> {
  ...
  if (EMPTY_SET_RETRY && emptyResult.rows.length > 0) {
    ...
    for (const emptySet of emptyResult.rows) {
      try {
        console.log(
          `Retrying: ${emptySet.name} (${emptySet.pokemontcg_io_set_id})`
        );
  ...
  console.log(
    `Current progress: ${progress.lastSetIndex} sets already imported\n`
  );
  ...
  for (let index = startIndex; index < sets.length; index++) {
    ...
    try {
      ...
      console.log(
        `  📊 Running totals: ${setsProcessed} sets, ${totalCards} cards, ${cardsWithImages} with images`
      );
    } catch (error) {
  ...
  const finalWithImages = await query(
...
async function verifyKnownCards(): Promise<void> {
  ...
  for (const card of testCards) {
    ...
    if (result.rows.length > 0) {
      ...
      console.log(
        `   Found: ${result.rows[0].card_name} #${result.rows[0].card_number}`
      );

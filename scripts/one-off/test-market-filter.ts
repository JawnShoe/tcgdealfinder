...
console.log("\n3. Expected behavior:");
console.log(
  "   ✓ normalizeMarketCode('all') should return 'all' (not 'EBAY_US')"
);
...
if (allNormalized === "all" && defaultFilter === "all") {
  ...
  if (allNormalized !== "all") {
    console.log(
      `   - normalizeMarketCode("all") returned "${allNormalized}" instead of "all"`
    );
  }
  if (defaultFilter !== "all") {
    console.log(
      `   - DEFAULT_MARKET_FILTER is "${defaultFilter}" instead of "all"`
    );
  }
}

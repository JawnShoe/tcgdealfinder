...
async function testSetup() {
  ...
  console.log(
    "✅ catalog_sets columns:",
    setsCheck.rows.map((r) => r.column_name).join(", ")
  );
  console.log(
    "✅ catalog_cards columns:",
    cardsCheck.rows.map((r) => r.column_name).join(", ")
  );
  ...
  const cardsWithImages = await query(

...
async function verifyMarketDisplay() {
  ...
  const hasQuestionMarks = distResult.rows.some(
    (row) => formatMarket(row.market).compactLabel === "??"
  );
  ...
  for (const row of distResult.rows) {
    ...
    for (const sample of samples.rows) {
      console.log(
        `  - ${sample.listing_id} | ${sample.currency} | ${sample.title.substring(0, 60)}...`
      );
    }
  }

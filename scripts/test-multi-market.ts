...
import { getFXRate } from "../lib/rebuild/scripts/fxRates";
...
async function testMarket(market: MarketCode, limit: number = 10) {
  ...
  try {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      console.log(
        `    Price: ${listing.priceCurrency} ${listing.priceCad?.toFixed(2) ?? "N/A"}`
      );
      console.log(
        `    Shipping: ${listing.shippingCurrency ?? listing.priceCurrency} ${listing.shippingCad?.toFixed(2) ?? "N/A"}`
      );
...
async function checkDatabase() {
  ...
  for (const row of sampleListings.rows) {
    console.log(
      `  ${row.market} | ${row.currency} ${row.total_native} → USD $${row.total_usd} (rate: ${row.fx_rate_to_usd})`
    );
  }
}

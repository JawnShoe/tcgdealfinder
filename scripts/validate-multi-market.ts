...
async function validate() {
  ...
  `);
  console.log(`   ${fxTable.rows[0].count > 0 ? '✅' : '❌'} fx_rates table exists`);
  
  const currencyCol = await query(`
    SELECT COUNT(*) as count FROM information_schema.columns
    WHERE table_name = 'listings' AND column_name = 'currency';
  `);
  console.log(`   ${currencyCol.rows[0].count > 0 ? '✅' : '❌'} listings.currency column exists`);
  
  const totalUsdCol = await query(`
    SELECT COUNT(*) as count FROM information_schema.columns
    WHERE table_name = 'listings' AND column_name = 'total_usd';
  `);
  console.log(`   ${totalUsdCol.rows[0].count > 0 ? '✅' : '❌'} listings.total_usd column exists`);
  ...
  `);
  console.log(`   ${oldConstraint.rows[0].count === 0 ? '✅' : '⚠️ '} Old UNIQUE(listing_id) ${oldConstraint.rows[0].count === 0 ? 'removed' : 'still exists'}`);
  ...
  `);
  console.log(`   ${newConstraint.rows[0].count > 0 ? '✅' : '❌'} New UNIQUE(listing_id, market) ${newConstraint.rows[0].count > 0 ? 'exists' : 'missing'}`);
  ...
  if (convertedCount.rows[0].count > 0) {
    ...
    for (const row of sample.rows) {
      console.log(`     ${row.market} | ${row.currency} ${Number(row.total_native).toFixed(2)} × ${row.fx_rate_to_usd} = USD $${Number(row.total_usd).toFixed(2)}`);
    }
  }
  ...
  console.log("  1. Run: npx tsx scripts/test-multi-market.ts --market EBAY_CA --limit 5");
  console.log("  2. Run: npx tsx scripts/update-listings.ts --market EBAY_CA --limit 10");
  console.log("  3. Check /cards/[cardId] UI - Market dropdown should show all 4 markets");
  console.log("  4. Verify sorting uses total_usd for cross-market comparison\n");
}
